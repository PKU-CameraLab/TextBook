...
def _test():
    ...
    for (idx, image_name) in enumerate(image_list): # NOTE: save all results
        # read image
        image = cv2.imread(image_name)
        # crop face and generate mask of face
        # aligned, mask, im, landmark = mg.align(image, size=(M, M))[0]
        # aligned, mask, im, landmark = mg.align(image, size=(M, M), warp=False, crop=False)[0]
        im = image # NOTE: mask have some problems, disable for now
        # resize
        ...
        # diff = (mask // 255) # NOTE: mask have some problems, disable for now
        diff = 1
        n_out2 = (n_out2+1)/2 * diff # NOTE: normal back to [0, 1]
        ...
        Ishd = cv2.cvtColor(Ishd, cv2.COLOR_RGB2BGR) # NOTE: abandon monochromatic light assumption
        ...
        # save result
        # NOTE: save all results now
        cv2.imwrite('data/%d_shading.png' %(idx+1), convert(Ishd)) 
        cv2.imwrite('data/%d_Albedo.png' %(idx+1), convert(al_out2))
        cv2.imwrite('data/%d_Recon.png' %(idx+1), convert(Irec))
        cv2.imwrite('data/%d_Normal.png' %(idx+1), convert(n_out2))
        # if cv2.waitKey(0) == 27:
        #     exit()
